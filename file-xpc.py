"""
MoMo Fraud SMS Detector — Self-contained Flask web app.
Trains the model on first startup, then serves the web UI and REST API.
No separate training step needed — just deploy and run.
"""
...
def preprocess(text: str) -> str:
    """5-step NLP pipeline: lowercase → strip → tokenise → stopwords → lemmatise."""
    text   = text.lower()
...
# ── Dataset Generation (Ghanaian MoMo context) ────────────────────────────────
FRAUD_TEMPLATES = [
...
HAM_TEMPLATES = [
    ...
    "Cash deposit of GHS {amount} received at branch Thank you for banking with GCB",
...
def build_dataset(n=600) -> pd.DataFrame:
    rows = (
...
def train():
    ...
    # TF-IDF vectoriser — exact parameters from Chapter 4.5 of the report
    ...
    # Train all three classifiers and pick the best (Logistic Regression per report)
    candidates = {
    ...
    for name, clf in candidates.items():
        ...
        results[name] = {'model': clf, 'accuracy': acc, 'precision': prec,
                         'recall': rec, 'f1': f1}
    ...
    # Return metrics for the /metrics endpoint
    return {name: {k: round(v, 4) for k, v in vals.items() if k != 'model'}
            for name, vals in results.items()}, best_name

# Train on startup if models don't exist yet
if os.path.exists(CLF_PATH) and os.path.exists(VEC_PATH):
    print("Loading saved model...")
    classifier = joblib.load(CLF_PATH)
    vectorizer = joblib.load(VEC_PATH)
    _metrics   = {}
    _best_name = "Logistic Regression"
else:
...
@app.route('/metrics')
def metrics():
    return jsonify({'best_model': _best_name, 'results': _metrics})
